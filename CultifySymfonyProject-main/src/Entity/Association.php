...
private ?string $siteWeb = null;
    ...
    public function getMontantActuel(): float
    {
        ...
        foreach ($this->IdDon as $don) {
            if ($don->getStatus() === 'confirme') {
                $montantTotal += $don->getMontant() ?? 0; // Gestion des montants null
            }
        }
        return $montantTotal;
    }
    ...
    {
        if (!$this->montantDesire || $this->montantDesire <= 0) {
            return 0; // Éviter division par zéro
        }
        $pourcentage = ($this->getMontantActuel() / $this->montantDesire) * 100;
        return min(100, max(0, $pourcentage)); // Limiter entre 0 et 100
    }
